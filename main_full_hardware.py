...
def setup_gpio():
    ...
    GPIO.output(BUZZER_PIN, GPIO.HIGH)  # Active low - HIGH = OFF
...
def buzz(duration=0.1):
    """Sound the buzzer (active-low logic)"""
    GPIO.output(BUZZER_PIN, GPIO.LOW)   # LOW = ON
    time.sleep(duration)
    GPIO.output(BUZZER_PIN, GPIO.HIGH)  # HIGH = OFF
    print(f"[BUZZER] {duration}s")
...
def monitor_buttons_and_sensors():
    ...
    lcd_string(f"Machine: {TEAM_NAME}", LCD_LINE_1)
    lcd_string("Ready", LCD_LINE_2)
    print("[MONITOR] Monitoring started")
    print(f"[LCD] Machine: {TEAM_NAME} / Ready\n")
    ...
    try:
        while True:
            ...
            if (time.time() - last_sensor_trigger > 20 and not downtime_triggered 
                ...
                lcd_string("Downtime Alert!", LCD_LINE_1)
                lcd_string("Select Event", LCD_LINE_2)
                print("[LCD] Downtime Alert! / Select Event")
                buzz(0.3)
            ...
            if (material_change_active and 
                ...
                lcd_string("Downtime Alert", LCD_LINE_2)
                print("[LCD] Timeout! / Downtime Alert")
                buzz(0.2)
